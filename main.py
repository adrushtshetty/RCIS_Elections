...
def admin():
    ...
    df2 = pd.read_csv(r'resSportsCaptain.csv')
    ...
    if (flag):
        nSPC = 1
        sSPC = ""
        msgSPC = str((resn[resv.index(max(resv))], "with total votes of ", max(resv)))
    else:
        var = resv
        unique_entries = set(var)
        indices = {value: [i for i, v in enumerate(var) if v == value] for value in unique_entries}
    ...
    rabSPC = ""
    for k, v in dict1.items():
        rabSPC += str((k + " with total votes of:", v, "|||"))
    ...
    return render_template("admin.html", msgSent='{}'.format(msg), rabSent='{}'.format(rab), no='{}'.format(n),
                           ...
                           noVC='{}'.format(nVC), sSentVC='{}'.format(sVC), msgSentSPC='{}'.format(msgSPC),
                           rabSentSPC='{}'.format(rabSPC), noSPC='{}'.format(nSPC), sSentSPC='{}'.format(sSPC))
...
def add():
    ...
    r = [vote_spc]
    with open("resSportsCaptain.csv", 'a') as f_object:
        writer_object = writer(f_object)
        writer_object.writerow(r)
        f_object.close()
